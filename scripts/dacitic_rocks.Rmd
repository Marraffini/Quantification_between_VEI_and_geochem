...
dacitic_rocks_dtb_csv2 = "/Users/annad/Documents/NZ Internship/all georoc databases/dacitic rocks/csv2"

csv2 = list.files(dacitic_rocks_dtb_csv2, pattern = "\\.csv$")

csv2_path = file.path(dacitic_rocks_dtb_csv2, csv2)
...
dacitic_rocks = lapply(csv2_path, trim_rocks_csv2)

csv2_name = sub("\\.csv$", "", csv2)

names(dacitic_rocks) = csv2_name


lapply(dacitic_rocks, nrow)
lapply(dacitic_rocks, head)
...
dacitic_references = lapply(csv2_path, references_csv2)

csv2_name = sub("\\.csv$", "", csv2)

names(dacitic_references) = csv2_name


#sapply(andesitic_references, nrow)
lapply(dacitic_references, head)
...
dacitic_references = lapply(dacitic_references, keep_between_references_criteria)


sapply(dacitic_references, head)
...
dacitic_references = lapply(dacitic_references, function(df) {
...
references_dacitic = dacitic_references

dacitic_references = bind_rows(references_dacitic, .id = "SOURCE")
...
dacitic_references <- dacitic_references %>%
...
#write.csv(dacitic_references, file = "/Users/annad/Documents/NZ Internship/references/dacitic rocks/dacitic_references.csv", row.names = FALSE)
...
done with the references for now
...
dacitic_rocks <- lapply(dacitic_rocks, select_relevant_columns, keep = columns_to_keep)

colnames(dacitic_rocks$dacit)
...
dacitic_rocks <- lapply(dacitic_rocks, function(df) {
...
dacitic <- bind_rows(dacitic_rocks, .id = "SOURCE")
...
# no need to write
#write.csv(dacitic, file = "/Users/annad/Documents/NZ Internship/georoc rock databases/dacitic_rocks.csv", row.names = FALSE)
...
dacitic_rocks_tri has only the major oxides and the citations.
...
class(dacitic_rocks_tri)
#View(basaltic_rocks_tri)


dacitic_rocks_tri_sum_fe = dacitic_rocks_tri %>% 
  mutate(sum_fe =
           case_when(
...
dacitic_rocks_tri_sum_fe is the same as dacitic_rocks_tri but with the column sum_fe in addition. 
...
dacitic_ref_tot_oxides <- dacitic_rocks_tri_sum_fe

dacitic_ref_tot_oxides$`Sum Oxides` <- rowSums(dacitic_ref_tot_oxides[, oxide_with_only_feot], na.rm = TRUE)
...
dacitic_ref_tot_oxides = dacitic_ref_tot_oxides %>% dplyr::select(-c("FEOT(WT%)", "FEO(WT%)", "FE2O3T(WT%)", "FE2O3(WT%)"))
...
dacitic_ref_tot_oxides is the same as dacitic_rocks_tri_sum_fe but with Sum Oxides in addition and without the Fe.
...
dacitic_oxides_complete is the same dacitic_oxides_complete but with all completed oxides.
...
dacitic_oxides_complete <- dacitic_oxides_complete %>%
...
df_ref = read_csv("/Users/annad/Documents/NZ Internship/references/dacitic rocks/dacitic_references.csv", show_col_types = FALSE)

# Séparer par groupes de citation
grouped_list <- split(dacitic_oxides_complete, dacitic_oxides_complete$CITATION_ID)
...
dacitic_rocks_all_info = final_df
...
```{r}
#write.csv(dacitic_rocks_all_info, file = "/Users/annad/Documents/NZ Internship/rocks all info/dacitic_rocks_all_info.csv", row.names = FALSE)
```
...
length(unique(dacitic_rocks_all_info$References)) #717 -> 709
length(dacitic_rocks_all_info$References) #6239 -> 6199
...
length(unique(dacitic_references$REFERENCES)) #3116
length(dacitic_references$REFERENCES) #3306

```

Above, I have 3116 unique different references in the references csv. 
After filtering, i should only have 717 references left.


```{r}
# lets create a df

dacitic_only_good_ref = dacitic_references
dacitic_only_good_ref = as.data.frame(dacitic_only_good_ref)


unique_ref <- unique(dacitic_rocks_all_info$CITATION_ID)
length(unique_ref)   # 836 normal


dacitic_only_good_ref <- dacitic_references %>% # dataframe
  filter(CITATION_ID %in% unique_ref) %>% 
  distinct(CITATION_ID, .keep_all = TRUE) # DONT FORGET OR ELSE ITS NOT THE SAME LENGTH

length(unique(dacitic_only_good_ref$CITATION_ID)) # 717
length(dacitic_only_good_ref$CITATION_ID) #717  


#write then comment
#write.csv(dacitic_only_good_ref, file = "/Users/annad/Documents/NZ Internship/rocks only good ref filtered/dacitic_only_good_ref.csv", row.names = FALSE)

```


dacitic_only_good_ref in this csv, all the references that are kept are the references where the oxides are complete.
...
dacitic_only_good_ref_filtered <- dacitic_only_good_ref[!grepl(pattern, dacitic_only_good_ref$REFERENCES, ignore.case = TRUE), ]

# it deletes 76 rows

#write.csv(dacitic_only_good_ref_filtered, file = "/Users/annad/Documents/NZ Internship/rocks only good ref filtered/dacitic_only_good_ref_filtered.csv", row.names = FALSE)
