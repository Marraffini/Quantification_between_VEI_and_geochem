...
probs_vei7_rf <- predict(model_rf, newdata = eruption_summary_vei7_stats[, X_mean_sd_cols], type = "prob")
pred_classes_vei7_rf <- predict(model_rf, newdata = eruption_summary_vei7_stats[, X_mean_sd_cols], type = "response")
...
ggplot(df_all, aes(x = Eruption, y = Predicted_VEI, color = Model)) +
  ...
  theme_minimal() +
  labs(
    title = "Predicted VEI for VEI 7 Eruptions (mean + cuberoot(sd))",
    x = "Eruption",
    y = "Predicted VEI"
  ) +
  scale_y_discrete(limits = vei_levels) +  # 👈 Affiche tous les VEI de 0 à 7
  scale_color_manual(values = c("Naive Bayes" = "steelblue", "Random Forest" = "darkorange")) +
...
dfs_all <- list(df_nb, df_rf, df_lda, df_svm, df_polr)
...
model_rf_aug <- randomForest(
  x = train_augmented[, X_mean_sd_cols],
...
probs_rf_aug <- predict(model_rf_aug, newdata = vei7_holdout[, X_mean_sd_cols], type = "prob")
pred_rf_aug <- predict(model_rf_aug, newdata = vei7_holdout[, X_mean_sd_cols], type = "response")
...
# graph for rf and nb ((with vei 7 in training set)
...
df_all$Predicted_VEI <- factor(df_all$Predicted_VEI, levels = vei_levels, ordered = TRUE)
df_all <- bind_rows(df_nb, df_rf)
df_all$Eruption <- factor(df_all$Eruption, levels = unique(df_all$Eruption))
