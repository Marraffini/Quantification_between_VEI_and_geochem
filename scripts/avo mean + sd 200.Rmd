...
# Transformer les matrices en data.frames bien nommés
df_nb <- data.frame(
  Eruption = vei7_holdout$Eruption[complete.cases(X_pred_nb)],
...
# Ajouter la proba max pour chaque prédiction
df_nb <- df_nb %>%
  rowwise() %>%
...
# Graphique avec label de la proba
ggplot(df_all, aes(x = Eruption, y = Predicted_VEI, color = Model)) +
  ...
  scale_y_discrete(limits = vei_levels) +  # 👈 Forcer l'affichage de tous les VEI
  scale_color_manual(values = c("Naive Bayes" = "steelblue", "Random Forest" = "darkorange")) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
