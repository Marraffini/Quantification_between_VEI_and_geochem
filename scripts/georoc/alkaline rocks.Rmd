...
In the type of "alkaline rocks", there are this sub type of rocks :

![subrock of alkaline rocks](/Users/annad/Documents/NZ Internship/Screenshot 2025-07-03 at 13.42.17.png)

For each subtype, there will be a csv, later in the script, we will link all those csv together.


From this script, we will have two csv : one with the references and one with the information of the samples. 
...
## Reading of all the csv of the alkaline subtype. 
Strangely, it downloaded either as a csv or csv2. So, put in separate folder "csv" and "csv2". 

```{r}
alkaline_rocks_dtb_csv = "/Users/annad/Documents/NZ Internship/all georoc databases/alkaline rocks/csv"
alkaline_rocks_dtb_csv2 = "/Users/annad/Documents/NZ Internship/all georoc databases/alkaline rocks/csv2"


csv = list.files(alkaline_rocks_dtb_csv, pattern = "\\.csv$")
csv2 = list.files(alkaline_rocks_dtb_csv2, pattern = "\\.csv$")

#print(csv)
#print(csv2)

csv_path = file.path(alkaline_rocks_dtb_csv, csv)
csv2_path = file.path(alkaline_rocks_dtb_csv2, csv2)
...
alkaline_references_csv = lapply(csv_path, references_csv)
alkaline_references_csv2 = lapply(csv2_path, references_csv2)

csv_name = sub("\\.csv$", "", csv)
csv2_name = sub("\\.csv$", "", csv2)

names(alkaline_references_csv) = csv_name
names(alkaline_references_csv2) = csv2_name

alkaline_references = c(alkaline_references_csv, alkaline_references_csv2)

sapply(alkaline_references, nrow)
sapply(alkaline_references, head)

#View(alkaline_references$phonolit)
...
class(alkaline_references$phonolit)
...
alkaline_references = lapply(alkaline_references, keep_between_references_criteria)
...
```{r}
alkaline_references = lapply(alkaline_references, function(df) {
...
I want to bind all the references from alkaline rocks together.


```{r}
references_alkaline = alkaline_references

alkaline_references = bind_rows(references_alkaline, .id = "SOURCE")

```



Let's delete the rows where "References:" and Criteria appear.

```{r}

alkaline_references <- alkaline_references %>%
  filter(
    !grepl("References:", Year, ignore.case = TRUE)) #&
    #!grepl("Criteria:", Year, ignore.case = TRUE)
  

#view(alkaline_references)
...
Let's write the csv.
```{r}
#write.csv(alkaline_references, file = "/Users/annad/Documents/NZ Internship/references/alkaline rocks/alkaline_references.csv", row.names = FALSE)

```



We are done for now with the references, let's work on the volcano data.
...
alkaline_rocks_csv = lapply(csv_path, trim_rocks_csv)
alkaline_rocks_csv2 = lapply(csv2_path, trim_rocks_csv2)

csv_name = sub("\\.csv$", "", csv)
csv2_name = sub("\\.csv$", "", csv2)

names(alkaline_rocks_csv) = csv_name
names(alkaline_rocks_csv2) = csv2_name

alkaline_rocks = c(alkaline_rocks_csv, alkaline_rocks_csv2)

sapply(alkaline_rocks, nrow)
lapply(alkaline_rocks, head)
...
alkaline_rocks <- lapply(alkaline_rocks, select_relevant_columns, keep = columns_to_keep)

#lapply(alkaline_rocks, names)



colnames(alkaline_rocks$analcimit)

```



lets force the columns to be of the same type (numeric or character)

```{r}

alkaline_rocks <- lapply(alkaline_rocks, function(df) {
...
class(alkaline_rocks$analcimit$`MNO(WT%)`)
class(alkaline_rocks$ankaratrit$Year)
class(alkaline_rocks$ankaratrit$`LOCATION COMMENT`)

```



lets fuse the csv of the samples together

```{r}

alkaline <- bind_rows(alkaline_rocks, .id = "SOURCE")
length(alkaline)
...
here we just want to keep the samples where :
- it is subaerial
- volcanic rock
- whole rock

and filter out when it has any variation of "dyke".
```{r}

alkaline_rocks_tri = alkaline %>%
  dplyr::filter(
...
alkaline_rocks_tri has only the major oxides and the citations.
(it is all the samples combined)
...
Sometimes, we have two different kind of iron, FeO(T) and Fe203(T), but we want the total/sum of iron. There are two different formulas to calculate that, depending on what we have. 
...
so we do that on all the samples
```{r}


class(alkaline_rocks_tri)
#View(alkaline_rocks_tri)


alkaline_rocks_tri_sum_fe = alkaline_rocks_tri %>% 
  mutate(sum_fe =
           case_when(
...
alkaline_rocks_tri_sum_fe is the same as alkaline_rocks_tri but with the column sum_fe in addition. 




### SUM OF ALL OXIDES (EXCLUDING LOI (Loss On Ignition))
...
alkaline_ref_tot_oxides <- alkaline_rocks_tri_sum_fe

alkaline_ref_tot_oxides$`Sum Oxides` <- rowSums(alkaline_ref_tot_oxides[, oxide_with_only_feot], na.rm = TRUE)

#write.csv(alkaline_ref_tot_oxides, file = "/Users/annad/Desktop/NZ Internship/alkaline_ref_total_oxides.csv")

#View(alkaline_ref_tot_oxides)
...
alkaline_ref_tot_oxides = alkaline_ref_tot_oxides %>% dplyr::select(-c("FEOT(WT%)", "FEO(WT%)", "FE2O3T(WT%)", "FE2O3(WT%)"))
...
alkaline_ref_tot_oxides is the same as alkaline_rocks_tri_sum_fe but with Sum Oxides in addition and without the Fe.
...
df <- alkaline_ref_tot_oxides %>%
  mutate(across(all_of(oxide_without_loi_and_iron), ~as.numeric(.)))

alkaline_oxides_complete <- df %>%
  filter(if_all(all_of(oxide_without_loi_and_iron), ~!is.na(.)))

names(alkaline_oxides_complete)
print(alkaline_oxides_complete)

```


alkaline_oxides_complete is the same alkaline_ref_tot_oxides but with all completed oxides.
...
alkaline_oxides_complete <- alkaline_oxides_complete %>%
...
alkaline_oxides_complete <- alkaline_oxides_complete %>%
  mutate(CITATION_ID = str_extract(CITATION, "\\[(\\d+)\\]"), # CITATION_ID and REFERENCES_ID are the same
         CITATION_ID = str_remove_all(CITATION_ID, "\\[|\\]"))
...
I will use the citation_id.

```{r}

df_ref = read_csv("/Users/annad/Documents/NZ Internship/references/alkaline rocks/alkaline_references.csv", show_col_types = FALSE)





# separate by citation groups
grouped_list <- split(alkaline_oxides_complete, alkaline_oxides_complete$CITATION_ID)
...
for (id in names(grouped_list)) {
  ...
  # take back the reference associated to the id
  reference <- df_ref %>%
  ...
  # add the column References with the ref on all rows
  block$References <- reference
  
  # put References to the end of the columns
...
alkaline_rocks_all_info = final_df
...
In alkaline_rocks_all_info, we have :
- every information
- sum of fe
- sum of major oxides
- citations
...
```{r}
#write.csv(alkaline_rocks_all_info, file = "/Users/annad/Documents/NZ Internship/rocks all info/alkaline_rocks_all_info.csv", row.names = FALSE)
```
...
Let's see how many unique references/citations in the csv of the samples we have :
...
length(unique(alkaline_rocks_all_info$References)) 
#length(alkaline_rocks_all_info$References) #5022 update 22/05 5008 --> not useful ? 
...
Let's filter from the references csv the references we are keeping, based on the samples csv.

```{r}

length(unique(alkaline_references$REFERENCES)) 
length(alkaline_references$REFERENCES)
...
alkaline_only_good_ref = alkaline_references
alkaline_only_good_ref = as.data.frame(alkaline_only_good_ref)


unique_ref <- unique(alkaline_rocks_all_info$CITATION_ID)
length(unique_ref)   #


alkaline_only_good_ref <- alkaline_references %>% # dataframe
  filter(CITATION_ID %in% unique_ref) %>% 
  distinct(CITATION_ID, .keep_all = TRUE) # DONT FORGET OR ELSE ITS NOT THE SAME LENGTH

length(unique(alkaline_only_good_ref$CITATION_ID)) # 
length(alkaline_only_good_ref$CITATION_ID) #


# write then comment
#write.csv(alkaline_only_good_ref, file = "/Users/annad/Documents/NZ Internship/rocks only good ref filtered/alkaline_only_good_ref.csv", row.names = FALSE)

```


alkaline_only_good_ref.csv in this csv, all the references that are kept are the references where the oxides are complete.
...
alkaline_only_good_ref = read_csv("/Users/annad/Documents/NZ Internship/rocks only good ref filtered/alkaline_only_good_ref.csv", show_col_types = FALSE)
...
alkaline_only_good_ref_filtered <- alkaline_only_good_ref[!grepl(pattern, alkaline_only_good_ref$REFERENCES, ignore.case = TRUE), ]

#write.csv(alkaline_only_good_ref_filtered, file = "/Users/annad/Documents/NZ Internship/rocks only good ref filtered/alkaline_only_good_ref_filtered.csv", row.names = FALSE)
