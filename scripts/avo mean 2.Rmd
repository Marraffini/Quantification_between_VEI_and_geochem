...
###################### Store results for current repetition and fold (only one fold and one split)
...
########## Store the results for one split one fold
...
# Prédiction
probs_vei7_rf <- predict(model_rf, newdata = eruption_summary_vei7[, X_mean_cols], type = "prob")
pred_classes_vei7_rf <- predict(model_rf, newdata = eruption_summary_vei7[, X_mean_cols], type = "response")
...
train_augmented <- eruption_summary_oxides_vei_volcano %>%
  ...
  bind_rows(vei7_to_add) %>%
  mutate(New_Vei = factor(New_Vei))  # RF ne prend pas ordered
