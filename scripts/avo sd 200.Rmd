...
p_facets_circles_border <- ggplot(
  ...
  scale_y_discrete(drop = FALSE) +
  labs(
    title = "Expected vs Predicted VEI (AVO - Cube Root SD)",
...
probs_mat <- predict(model_rf, data = eruption_summary_vei7_cubersd[, X_sd_cols])$predictions
...
#orf --> dont work idk why -> works now, but different code
...
train_orf <- eruption_summary_oxides_vei_volcano %>%
  dplyr::filter(New_Vei %in% 1:6) %>%
  dplyr::select(New_Vei, dplyr::all_of(X_sd_cols)) %>%
...
# --- 3) Prepare test data ---
test_orf <- eruption_summary_vei7_cubersd[, setdiff(names(train_orf), "New_Vei"), drop = FALSE]
ok <- complete.cases(test_orf)
eruption_ids <- eruption_summary_vei7_cubersd$Eruption[ok]
...
# --- Data prep (concise) ---
safe_features <- intersect(X_sd_cols, colnames(eruption_summary_oxides_vei_volcano))
...
vei7_holdout_clean <- eruption_summary_vei7_cubersd %>%
  dplyr::filter(!Eruption %in% c("Crater Lake 5750 BCE", "Santorini 1600 BCE", "Rinjani 1257")) %>%
...
test_orf <- vei7_holdout %>%
  dplyr::select(Eruption, dplyr::all_of(X_sd_cols))

ok <- complete.cases(test_orf[, X_sd_cols])
eruption_ids <- test_orf$Eruption[ok]
X_test <- test_orf[ok, X_sd_cols, drop = FALSE]
