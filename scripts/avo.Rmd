...
this code works but it doesn't include to keep the 'Ref traces' columns.
```{r, eval=FALSE}
# Trouver la position de la colonne "Total-majors"
idx_ref_majors <- which(names(avo) == "REF majors")

# Garder uniquement les colonnes jusqu'à "REF majors" inclus
avo <- avo[, 1:idx_ref_majors]
...
```{r}
nb_sample_by_eruption_with_volcano <- avo_with_confirmed_eruption %>%
  count(Volcano, Eruption, name = "Nb_echantillons") %>%
  arrange(desc(Nb_echantillons))

nb_eruption_by_volcano <- avo_with_confirmed_eruption %>%
  distinct(Volcano, Eruption) %>%  # Retirer les doublons Volcano–Eruption
  count(Volcano, name = "Nb_eruption") %>%  # Compter les éruptions par volcan
  arrange(desc(Nb_eruption))  # Trier du plus grand au plus petit


# no need
#write.csv(nb_eruption_by_volcano, "/Users/annad/Documents/NZ Internship/nb_eruption_by_volcano.csv", row.names = FALSE)
...
# Force the oxides to be numeric type

avo_with_confirmed_eruption <- avo_with_confirmed_eruption %>%
  mutate(across(all_of(oxydes_keywords), ~ as.numeric(.)))
...
avo_with_confirmed_eruption <- avo_with_confirmed_eruption %>%
  mutate(across(all_of(c(oxydes_keywords, "Total-majors")), extract_last_number))

# 2. select the oxides
oxydes_only <- avo_with_confirmed_eruption %>%
  dplyr::select(all_of(oxydes_keywords)) %>%
  as.data.frame()

# 3. Function with two objectives : imputation, normalisation or NA
impute_or_normalize <- function(x) {
  na_count <- sum(is.na(x))
  
  if (na_count == 1) {
    missing_pos <- which(is.na(x))
    sum_non_missing <- sum(x, na.rm = TRUE)
    valeur_imputee <- 100 - sum_non_missing
    #x[missing_pos] <- ifelse(valeur_imputee < 0, 0, valeur_imputee). ------> pb with the alr transformation otherwise
    x[missing_pos] <- ifelse(valeur_imputee < 1e-4, 1e-4, valeur_imputee)
    
  } else if (na_count == 2) {
    sum_non_missing <- sum(x, na.rm = TRUE)
    x <- x / sum_non_missing * 100
...
# 4. Apply to each row
oxydes_imputed <- as.data.frame(t(apply(oxydes_only, 1, impute_or_normalize)))
colnames(oxydes_imputed) <- oxydes_keywords

# 5. delete rows where there are still NAs
ligne_valide <- complete.cases(oxydes_imputed)
oxydes_imputed <- oxydes_imputed[ligne_valide, ]
avo_with_confirmed_eruption <- avo_with_confirmed_eruption[ligne_valide, ]

# 6. replace oxides in AVO
avo_with_confirmed_eruption <- avo_with_confirmed_eruption %>%
  dplyr::select(-all_of(oxydes_keywords)) %>%
  bind_cols(oxydes_imputed)

# 7. update "Total-majors" if NA
avo_with_confirmed_eruption <- avo_with_confirmed_eruption %>%
  mutate(`Total-majors` = ifelse(
    is.na(`Total-majors`),
    rowSums(across(all_of(oxydes_keywords)), na.rm = TRUE),
    `Total-majors`
  ))
...
oxydes_trop_faibles <- avo_somme_check %>%
  dplyr::filter(somme_oxydes < 96)

oxydes_trop_fortes <- avo_somme_check %>%
...
avo_with_confirmed_eruption2 <- avo_with_confirmed_eruption2 %>%
  ...
  #select(-somme_oxydes)  # Enlever la colonne temporaire si tu ne veux pas la garder
...
nb of volcano with existing eruptions

```{r}
number_of_volcanoes <- avo_with_confirmed_eruption2 %>%
  distinct(Volcano) %>%
  nrow()

print(number_of_volcanoes)

```

number_of_volcanoes volcanoes with existing eruptions
...
eruption_summary <- eruption_summary %>%
  dplyr::filter(!Eruption %in% eruptions_a_supprimer)
...
lets filter eruption summary out of avo_with_confirmed_eruption
...
# Résumé par éruption : moyenne des alr standardisées
eruption_summary <- avo_filtered %>%
  group_by(Eruption) %>%
  summarise(
    VEI = unique(VEI),  # on suppose 1 VEI par éruption
    n_samples = n(),
    across(
      all_of(oxydes),
...
I have 15 eruptions where i only have 1 sample. 
...
```{r}
# 1. Extraire la liste des noms de volcans depuis avo_alr_scaled
avo_volcano_names <- unique(avo_filtered$Volcano)

# 2. Filtrer les volcans du GVP qui apparaissent dans la liste des volcans d'AVO
